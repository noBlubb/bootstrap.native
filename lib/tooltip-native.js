...
var Tooltip = function (element, options) {
    ...
    this.duration = 150;
    this.options.duration = utils.isIE && utils.isIE < 10 ? 0 : (options.duration || this.duration);
    ...
    this.close = function (e) {
        ...
        timer = setTimeout(function () {
            if (self.tooltip && self.tooltip !== null) {
                utils.removeClass(self.tooltip, 'in');
                setTimeout(function () {
                    self.removeToolTip();
    ...
    this.styleTooltip = function () {
        var rect = this.link.getBoundingClientRect(), scroll = utils.getScroll(), // link rect | window vertical and horizontal scroll
        ...
        this.tooltip.className.indexOf(placement) === -1 && (this.tooltip.className = this.tooltip.className.replace(utils.tipPositions, placement));
    };
    this.updateTooltip = function () {
        this.styleTooltip();
        if (!utils.isElementInViewport(this.tooltip)) {
            placement = this.updatePlacement();
            this.styleTooltip();
        }
    };
    this.showTooltip = function () {
        !/\bin/.test(this.tooltip.className) && ( utils.addClass(this.tooltip, 'in') );
    };
    ...
    // init
    this.link.addEventListener(utils.mouseHover[0], this.open, false);
    this.link.addEventListener(utils.mouseHover[1], this.close, false);
